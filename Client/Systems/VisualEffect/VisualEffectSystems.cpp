...
namespace System {
    ...
    /**
     * @brief 
     * structure avec les information sur un type de particules à afficher
     * 
     */
    ...
    /**
     * @brief 
     * vecteur avec toutes les particules d'un type a afficher
     * 
     */
    ...
    /**
     * @brief 
     * One Visual Effect that produce an explosion, Client-side
     * 
     * @param r 
     * @param singletons 
     * @param texture 
     * @param pad 
     */
    void explosionEffect(Registry &r, EngineComponent::Singletons &singletons, std::string const &texture, EngineComponent::SpriteSheet const &pad)
    {
        for (auto const &pos : singletons.getSingleton<std::vector<PadaEngine::Position>>(EngineClient::UDP::SingletonsEnum::POS_EXPLOSION)) {
            explosions.push_back({pad, pos, texture});
            if (pos.x < 1900.0f)
                singletons.getSingleton<std::string>(EngineClient::UDP::EFFECT_NAME) = "hit";
        }
        for (int j = 0; j != explosions.size(); ++j) {
            ...
            r.textureCache.drawTextureRec<Element>(explosions.at(j).texture, {explosions.at(j).pos.x, explosions.at(j).pos.y}, explosions.at(j).sprite.rec);
        ...
        singletons.getSingleton<std::vector<PadaEngine::Position>>(EngineClient::UDP::SingletonsEnum::POS_EXPLOSION).clear();
    }

    /**
     * @brief 
     * Handle all the Visual Effect, Client-side
     * 
     * @param r 
     * @param singletons 
     */
        ...
        for (size_t i = 0; i < recs.size() && i < effectNames.size() && i < files.size(); ++i) {
            ...
            if (rec && effectName && file) {
                if (effectName.value().name == "basicExplosion")
                    explosionEffect(r, singletons, file.value().texture, rec.value());
            }
        }
    }
    ...
    /**
     * @brief 
     * Run the Visual Effect System, Client-side
     * 
     * @param r 
     * @param singletons 
     */
