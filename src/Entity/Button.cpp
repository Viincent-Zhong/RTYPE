...
namespace Entity {
    ...
    Button::Button(std::string const &file)
    {
    ...
    /**
     * @brief 
     * Set Buttons from a CSV file
     * 
     * @param r 
     * @param file 
     * @return std::vector<std::size_t> 
     */
    std::vector<std::size_t> Button::addEntitiesFromCSV(Registry &r, std::string const &file)
    {
        ...
        for (auto it : _content) {
            ...
            r.addComponent<EngineComponent::Interactible>(id, {std::atoi(it.at(11).c_str()) == 0 ? false : true});
            r.addComponent<EngineComponent::FunctionName>(id, {it.at(12)});
            r.addComponent<EngineComponent::EffectName>(id, {it.at(13)});
            std::stringstream sstream(it.at(14));
	        size_t result;
	        sstream >> result;
    ...
    /**
     * @brief 
     * Create Buttons based on previous data
     * 
     * @param r 
     * @return std::vector<std::size_t> 
     */
    std::vector<std::size_t> Button::addEntitiesFromCSV(Registry &r)
    {
        std::vector<std::size_t> ids;
    ...
    std::size_t Button::addEntityFromJSON(Registry &r, std::string const &name)
    {
        return std::size_t();
    }
    std::size_t Button::addEntityFromJSON(Registry &r, std::string const &file, std::string const &name)
    {
        return std::size_t();
    }
};